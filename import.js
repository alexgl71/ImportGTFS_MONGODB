...
// --- Parametri fissi per la ricerca (uguali per tutte le città) ---
const SEARCH_RADIUS      = 400;        // metri intorno a origine/destinazione
const TIME_FROM          = '08:00:00';
const TIME_TO            = '09:00:00';
const TRANSFER_MARGIN_SEC = 2 * 60;   // attesa minima al trasbordo
const MAX_DURATION_SEC    = 60 * 60;  // durata massima totale viaggio
...
async function enrichWithWalking(db, directResults, transferResults, origin, destination) {
...
async function findTransferRoutes(db, originStopIds, destStopIds, tripOriginSeq, directRouteIds, originTimeMap) {
  ...
  const legBDestTimes = await db.collection('daily_stop_times').find(
    { trip_id: { $in: legBTripIds }, stop_id: { $in: [...destSet] } },
    { projection: { _id: 0, trip_id: 1, stop_id: 1, stop_sequence: 1, arrival_time: 1 } }
  ).toArray();
...
async function findRoutesBetween(db, cfg) {
  const { origin, destination } = cfg;
  ...
  await enrichWithWalking(db, directResults, transferResults, origin, destination);
...
async function main() {
  ...
  const db = client.db(SEARCH_CONFIG.dbName);
